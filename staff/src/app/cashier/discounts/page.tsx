...
export default function CashierDiscountsPage() {
  ...
  useEffect(() => {
    // Load staff session for branch context
    ...
    const branchesQ = query(collection(db, "branches"));
    const unsubBranches = onSnapshot(branchesQ, (snapshot) => {
      const bList: any[] = [];
      snapshot.forEach((doc) => {
  ...
  // Branch filtered coupons: shows global coupons or coupons for the cashier's active branch
  const branchFilteredCoupons = useMemo(() => {
    const cashierBranchId = session?.branchId;
    return coupons.filter(c => {
      if (!cashierBranchId) return true;
      return !c.branchId || c.branchId === "global" || c.branchId === cashierBranchId;
    });
  }, [coupons, session?.branchId]);

  // KPI Calculations
  const totalCount = branchFilteredCoupons.length;
  const activeCount = branchFilteredCoupons.filter(isActive).length;
  const deactivateCount = branchFilteredCoupons.filter(isDeactivated).length;
  const expiredCount = branchFilteredCoupons.filter(isExpired).length;
  ...
  const filteredCoupons = useMemo(() => {
    return branchFilteredCoupons.filter((coupon) => {
      const matchesSearch =
        !searchQuery ||
  ...
  }, [branchFilteredCoupons, searchQuery, statusFilter]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = {
        ...
        branchId: formData.branchId || session?.branchId || "global",
        branchName: branchObj ? branchObj.name : formData.branchId === "global" ? "All Branches" : ""
      };
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-7xl mx-auto w-full pb-10">
      
      {/* 1. Four KPI Cards at Top (Identical to Superadmin Offers) */}
        ...
        {kpis.map((kpi) => (
          <Card 
            ...
            className={`cursor-pointer select-none rounded-2xl border-[3px] transition-all duration-300 hover:-translate-y-1 active:scale-[0.99] ${
              kpi.active 
                ? kpi.theme === "purple"
                  ? "border-[#7C3AED] ring-4 ring-[#7C3AED]/10 shadow-md bg-purple-50/10"
                  : kpi.theme === "emerald"
                  ? "border-emerald-500 ring-4 ring-emerald-500/10 shadow-md bg-emerald-50/10"
                  : kpi.theme === "orange"
                  ? "border-orange-500 ring-4 ring-orange-500/10 shadow-md bg-orange-50/10"
                  : "border-red-500 ring-4 ring-red-500/10 shadow-md bg-red-50/10"
                : "border-transparent hover:border-slate-200 shadow-[0_4px_25px_rgb(0,0,0,0.02)]"
            }`}
          >
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="text-[14px] font-black text-[#0F172A] tracking-tight mb-1">{kpi.title}</p>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger render={<Button onClick={() => openDialog("add")} className="h-[46px] rounded-xl px-6 bg-[#A855F7] hover:bg-[#9333EA] text-white shadow-sm hover:-translate-y-0.5 transition-all w-full sm:w-auto font-bold text-[14px] border-0 flex items-center justify-center cursor-pointer" />}>
          ...
          <DialogContent showCloseButton={false} className="w-full sm:max-w-4xl overflow-y-auto max-h-[90vh] p-0 gap-0 bg-[#F8F9FA] rounded-[2rem] shadow-[rgba(0,0,0,0.15)_0px_10px_40px] outline-none border-0">
            ...
            <form onSubmit={handleSave} className="p-8 pt-4 pb-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="space-y-2">
                  <Label className="text-[12px] font-bold text-slate-700 uppercase tracking-wider">Branch Assignment</Label>
                  <div className="relative group">
                    <Select value={formData.branchId || "global"} onValueChange={(v) => setFormData({...formData, branchId: v})}>
                      <SelectTrigger className="h-[48px] w-full pl-5 pr-4 bg-white border border-slate-200 focus:ring-1 focus:ring-[#A855F7] focus:border-[#A855F7] rounded-xl transition-all font-medium text-[14px] shadow-sm text-slate-600 data-[state=open]:border-[#A855F7]">
                        <SelectValue placeholder="Select Branch" />
                      </SelectTrigger>
                      <SelectContent className="rounded-xl border-slate-100 shadow-xl shadow-slate-200/50 p-1.5 max-h-[250px]">
                        <SelectItem value="global" className="cursor-pointer hover:bg-slate-50 rounded-lg font-bold text-[#7C3AED]">All Branches (Global)</SelectItem>
                        {branches.map(b => (
                  ...
                  >
                    {formData.image ? (
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-[11px] leading-5 text-slate-400">PNG, JPG, GIF up to 5MB</p>
                      </div>
                    )}
                  </div>
                  <input 
              ...
              <div className="flex justify-end items-center gap-4 mt-8">
                ...
                <Button type="submit" disabled={isSubmitting || isUploading} className="h-[48px] px-8 rounded-xl font-bold bg-[#A855F7] hover:bg-[#9333EA] text-white shadow-[0_4px_15px_rgba(168,85,247,0.4)] hover:shadow-[0_6px_20px_rgba(168,85,247,0.5)] hover:-translate-y-0.5 transition-all border-0 cursor-pointer">
              ...
              return (
                ...
                >
                  <div className="absolute top-3 right-3 z-20" onClick={(e) => e.stopPropagation()}>
                    <DropdownMenu>
                      <DropdownMenuTrigger render={<Button variant="ghost" size="icon" className="h-8 w-8 text-slate-700 bg-white/90 shadow-sm backdrop-blur-sm hover:bg-white rounded-full flex items-center justify-center" />}>
                  ...
                  <div className="w-full h-36 relative overflow-hidden bg-slate-100 shrink-0">
                    {coupon.image ? (
                  ...
                  <CardContent className="pt-5 px-5 pb-5 relative flex flex-col flex-1">
                    ...
                    <div className="inline-block px-3 py-1 bg-purple-50 rounded-md border border-purple-100 border-dashed font-mono font-bold text-[#7C3AED] text-[13px] mb-4 w-fit">
                      {coupon.code}
                    </div>
                    ...
                    <div className="space-y-1.5 mt-auto pt-3 border-t border-slate-100">
                      ...
                      <div className="flex items-center justify-between text-[13px]">
                        <span className="text-slate-500 flex items-center gap-1.5 font-medium"><TrendingUp className="w-3.5 h-3.5 text-slate-400" /> Usage</span>
                        <span className="font-bold text-slate-700">{coupon.usage || "0 / Unlimited"}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
            ...
            <>
              ...
              <div className="px-8 pb-8 -mt-8 relative z-20">
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                  ...
                  <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                    <span className="text-[13px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2"><TrendingUp className="w-4 h-4 text-slate-300" /> Total Usage</span>
                    <span className="text-[15px] font-black text-slate-800">{viewDetailsItem.usage || "0 / Unlimited"}</span>
                  </div>
                  <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                    <span className="text-[13px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2"><Store className="w-4 h-4 text-slate-300" /> Branch</span>
                    <span className="text-[15px] font-black text-slate-800">{viewDetailsItem.branchName || (viewDetailsItem.branchId === "global" ? "All Branches" : viewDetailsItem.branchId || "All Branches")}</span>
                  </div>
