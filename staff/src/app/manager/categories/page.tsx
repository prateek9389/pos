...
export default function MenuCategoriesPage() {
  ...
  useEffect(() => {
    ...
    // We should ideally query by restaurantId, but if it doesn't exist we just fetch all for now
    const q = session.restaurantId 
      ? query(collection(db, "menuCategories"), where("restaurantId", "==", session.restaurantId))
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          <Link href="/manager/menu" className={buttonVariants({ variant: "outline", className: "w-full sm:w-auto border-[#7C3AED] text-[#7C3AED] hover:bg-[#7C3AED] hover:text-white transition-colors h-[52px] rounded-full px-8 font-bold" })}>
            View All Items
          </Link>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger render={<Button onClick={() => openDialog("add")} className="h-[52px] rounded-full px-8 bg-[#7C3AED] hover:bg-[#6D28D9] text-white shadow-[0_4px_15px_rgba(168,85,247,0.3)] hover:shadow-[0_6px_20px_rgba(168,85,247,0.4)] hover:-translate-y-0.5 transition-all w-full sm:w-auto font-bold text-[15px] border-0" />}>
            ...
            <DialogContent showCloseButton={false} className="w-full sm:max-w-xl overflow-y-auto max-h-[90vh] p-0 gap-0 bg-[#F8F9FA] rounded-[2rem] shadow-[rgba(0,0,0,0.15)_0px_10px_40px] outline-none border-0">
              <div className="relative overflow-hidden bg-gradient-to-br from-[#7C3AED] to-[#7C3AED] px-8 pt-8 pb-24 shrink-0">
              ...
              <form onSubmit={handleSave} className="p-8 pt-4 pb-8">
                <div className="grid gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      <Tag className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-[#7C3AED] transition-colors pointer-events-none" />
                      <Input 
                        id="name" 
                        value={formData.name || ""}
                        onChange={(e) => setFormData({...formData, name: e.target.value})}
                        placeholder="e.g. Desserts" 
                        className="h-[48px] pl-11 bg-white border-slate-200 focus-visible:ring-1 focus-visible:ring-[#7C3AED] focus-visible:border-[#7C3AED] focus-visible:text-slate-900 rounded-xl transition-all font-medium text-[14px] shadow-sm" 
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      className="w-full min-h-[80px] p-3 bg-white border border-slate-200 focus:outline-none focus:ring-1 focus:ring-[#7C3AED] focus:border-[#7C3AED] text-slate-900 rounded-xl transition-all font-medium text-[14px] shadow-sm resize-y" 
                    />
                  </div>
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <div className="flex items-center gap-6">
                      <label className="flex items-center gap-2.5 cursor-pointer group">
                        ...
                        <span className="text-[14px] font-medium text-slate-700 group-hover:text-[#7C3AED] transition-colors">Active</span>
                      </label>
                      <label className="flex items-center gap-2.5 cursor-pointer group">
                        <div className="relative flex items-center justify-center">
                        ...
                        <span className="text-[14px] font-medium text-slate-700 group-hover:text-[#7C3AED] transition-colors">Inactive</span>
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div 
                      ...
                      className="border-2 border-dashed border-slate-200 hover:border-[#7C3AED] hover:bg-[#F4EBFF] bg-white rounded-2xl p-6 flex flex-col items-center justify-center gap-3 transition-colors cursor-pointer group h-48 relative overflow-hidden"
                    >
                      {formData.image ? (
                        <>
                        ...
                        <>
                          <div className="w-12 h-12 rounded-full bg-slate-50 group-hover:bg-white flex items-center justify-center shadow-sm transition-colors">
                            <ImageIcon className="w-5 h-5 text-slate-400 group-hover:text-[#7C3AED] transition-colors" />
                          </div>
                          <div className="text-center">
                            <p className="text-[13px] font-bold text-[#7C3AED]">Click to upload <span className="text-slate-500 font-medium">or drag and drop</span></p>
                ...
                <div className="flex justify-end items-center gap-4 mt-8">
                  ...
                  <Button type="submit" disabled={isSubmitting || isUploading} className="h-[48px] px-8 rounded-xl font-bold bg-[#7C3AED] hover:bg-[#6D28D9] text-white shadow-[0_4px_15px_rgba(168,85,247,0.4)] hover:shadow-[0_6px_20px_rgba(168,85,247,0.5)] hover:-translate-y-0.5 transition-all border-0">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
          {filteredCategories.map((category) => (
            <Link href={`/manager/menu?category=${category.id}`} key={category.id}>
                    ...
                    <div className="mt-auto pt-2">
                      ...
                      <div className="flex items-center justify-between pb-1">
                        <button 
                          ...
                          className="text-[12px] font-bold tracking-widest text-[#002D5B] hover:text-[#7C3AED] uppercase transition-colors"
                        >
                          Edit
                        </button>
                        <button 
