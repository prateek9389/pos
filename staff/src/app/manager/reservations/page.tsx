...
export default function ManagerReservations() {
  ...
  const filtered = reservations.filter((r) => {
    ...
    const matchStatus = 
      ...
      (filterStatus === "Pending" && statusLower === "upcoming");
  ...
  const kpis = [
    ...
    { 
      id: "Confirmed",
      label: "Confirmed", 
      value: reservations.filter(r => (r.status || "").toLowerCase() === "confirmed").length, 
    ...
    { 
      id: "Pending",
      label: "Pending", 
      value: reservations.filter(r => (r.status || "").toLowerCase() === "pending" || (r.status || "").toLowerCase() === "upcoming").length, 
    ...
    { 
      id: "Cancelled",
      label: "Cancelled", 
      value: reservations.filter(r => (r.status || "").toLowerCase() === "cancelled").length, 
      color: "text-red-600", 
      bg: "bg-red-100", 
      theme: "red",
      icon: <XCircle className="w-7 h-7 text-red-600" />,
      active: filterStatus === "Cancelled",
      onClick: () => setFilterStatus(prev => prev === "Cancelled" ? "all" : "Cancelled")
    },
  ];
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map((k) => {
          ...
          return (
            ...
            >
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${k.bg}`}>
                {k.icon}
              </div>
              <div className="flex flex-col">
                <p className="text-[14px] font-black text-[#0F172A] tracking-tight">{k.label}</p>
                <h3 className="text-[30px] font-black text-slate-900 leading-none mt-1.5">{k.value}</h3>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-3 rounded-2xl border border-slate-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
        <div className="relative group w-full sm:w-[360px]">
          ...
          <input 
            type="text" 
            placeholder="Search by customer, phone, date..." 
            value={searchTerm}
        ...
        {branches.length > 1 && (
          <div className="w-full sm:w-auto">
            <Select value={filterBranch} onValueChange={setFilterBranch}>
              <SelectTrigger className="w-full sm:w-[180px] h-[42px] bg-white border border-slate-200 rounded-xl font-bold text-slate-700">
