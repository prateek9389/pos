...
async function run() {
  ...
  const collections = ['staff', 'menuCategories', 'menuItems', 'inventory', 'tables', 'orders', 'reservations', 'payments', 'coupons'];
  for (const coll of collections) {
    ...
    for (const document of snapshot.docs) {
      if (!document.data().branchId || document.data().branchId === 'global' || document.data().branchId === 'all' || document.data().branchId.includes('Premium')) {
